...
export default function GC26AttendancePage() {
  ...
  const onScanSuccess = async (decodedText: string) => {
    ...
    const isValidFormat = /^(GC26|GG|GC)\d{2,}$/i.test(decodedText);
    if (!isValidFormat) {
    ...
    try {
      const res = await fetch(`/api/admin/gc26/attendance?code=${decodedText}`);
  ...
  const confirmAttendance = async () => {
    try {
      const res = await fetch(`/api/admin/gc26/attendance`, {
        method: "PATCH",
  ...
  return (
    <div className="flex flex-col items-center py-4 space-y-4">
      ...
      <button
        ...
        className="px-6 py-3 bg-purple-700 hover:bg-purple-800 transition text-white font-semibold rounded-xl shadow-md cursor-pointer"
      >
        Start QR Scan
      </button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-[420px] p-5 relative">
            <button
              ...
              className="absolute right-4 top-4 text-gray-500 hover:text-black font-bold cursor-pointer"
            >
              ✕
            </button>
            ...
            <div className="mt-4 flex gap-2">
              <button
                onClick={() => setShowScanner(false)}
                className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-xl font-medium cursor-pointer"
              >
                Cancel
              </button>
            </div>
