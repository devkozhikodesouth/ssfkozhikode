...
export async function GET() {
  try {
    ...
    const stats = await GrandGathering.aggregate([
      {
        $group: {
          _id: {
    ...
    const divisionSummary = divisions.map((d) => {
      ...
      return {
        ...
        totalRegistered: c.divisionRegistered + c.sectorRegistered,
    ...
    const overall = await GrandGathering.aggregate([
      {
        $group: {
          _id: null,
    ...
    console.error("Grand Gathering division summary error:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to fetch division summary for Grand Gathering",
      },
      { status: 500 }
    );
  }
