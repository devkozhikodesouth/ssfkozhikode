...
/* ----------------------------------------------------
 * GET : Fetch user by mobile for Grand Conclave 26
 * -------------------------------------------------- */
export async function GET(req: Request) {
  try {
    ...
    const user: any = await GrandConclave26.findOne({ mobile })
...
/* ----------------------------------------------------
 * POST : Register new user for Grand Conclave 26
 * -------------------------------------------------- */
export async function POST(req: Request) {
  try {
    ...
    /* ---------- Prevent Duplicate ---------- */
    const exists = await GrandConclave26.findOne({ mobile });
    if (exists) {
      return NextResponse.json(
    ...
    /* ---------- Create Record ---------- */
    const user = await GrandConclave26.create({
      name,
      mobile,
      organizationLevel,
      designation,
